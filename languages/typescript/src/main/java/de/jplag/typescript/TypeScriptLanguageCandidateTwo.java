...
/**
 * ASSIGN are removed. The intuition is that they behave very differently from Java or similar JPlag tokens
 */
@MetaInfServices(Language.class)
public class TypeScriptLanguageCandidateTwo extends AbstractGeneralTypeScriptLanguage {
    ...
    public List<Token> parseGeneral(Set<File> files, boolean normalize) throws ParsingException {
        ...
        return specificTokens.stream().map(token -> {
            TokenType type = switch (token.getType()) {
                ...
                case TypeScriptTokenType.ASSIGNMENT -> null;
